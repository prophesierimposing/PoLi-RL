...
class GRPOTrainer(RLHFTrainerMixin, SwiftMixin, HFGRPOTrainer):
        ...
        if reward_funcs:
            for i, reward_func in enumerate(reward_funcs):
                if reward_func in orms:
                    ...
                    if 'num_generations' in reward_func_args:
                        reward_func_kwargs['num_generations'] = args.num_generations
    ...
    def _prepare_inputs(self, generation_batch: dict[str, Union[torch.Tensor,
        ...
        if mode == 'train':
            ...
            if self._step % generate_every == 0 or self._buffered_inputs is None:
                ...
                logger.info(f'_prepare_inputs: Generated completions for {len(generation_batch)} inputs.')
                self._buffered_inputs = generation_batch 
                logger.info(f'_prepare_inputs: self._buffered_inputs length: {len(self._buffered_inputs)}.')
            inputs = self._buffered_inputs[self._step % self.args.steps_per_generation] 
            self._step += 1
        else:
            inputs = self._generate_and_score_completions(generation_batch)
        logger.info(f'_prepare_inputs: Prepared inputs with {len(inputs)} items.')
        return inputs
    ...
    def _prepare_batch_inputs(self, inputs: InputsType, rewards: torch.Tensor) -> List[InputsType]:
        ...
        logger.info(f'_prepare_batch_inputs: Advantages shape: {advantages.shape}, mean: {advantages.mean().item()}, std: {advantages.std().item()}')
